...
from utils import p_utils as utils


def main(event,context):
    #for i in event.Records:
        p_data = handler(event,context,'pshtt2')
        

        s_data = handler(event,context,'sslyze2')
        total_scans = {'pshtt':p_data,'sslyze':s_data}
        print(total_scans)
        
# Central handler for all Lambda events.
def handler(event, context,module):
